...
void isr_install() {
    ...
    // Remap the PIC
    // Because IRQs 0 to 7 conflict with the CPU exception which are reserved by
    // Intel up until 0x1F (it was an IBM design mistake)
    // A common choice is to move them to the beginning of the available range
    // (IRQs 0..0xF -> INT 0x20..0x2F). For that, we need to set the master PIC's
    // offset to 0x20 and the slave's to 0x28.
    ...
    port_byte_out(PIC1_DATA, 0x20); // ICW2: Master PIC vector offset
    port_byte_out(PIC2_DATA, 0x28); // ICW2: Slave PIC vector offset
...
void irq_handler(registers_t r){
  ...
  if (r.int_no >= 40) port_byte_out(PIC2_COMMAND, 0x20); /* slave */
  port_byte_out(PIC1_COMMAND, 0x20); /* master */
